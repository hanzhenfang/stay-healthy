...
export class ToastCreator {
  ...
  present() {
    ...
    const durationTime =
      this.options.duration || DEFAULT_TOAST_OPTIONS.duration;
    setTimeout(() => {
      this.dismiss();
    }, durationTime);
  }
  ...
  dismiss() {
    if (this.container) {
      const toastWrapperDOM = this.container.querySelector(
        "#toastWrapper"
      ) as HTMLDivElement;
      toastWrapperDOM.style.opacity = "0";
      toastWrapperDOM.style.transition = "opacity 0.4s ease-in";
      setTimeout(() => {
        if (this.container) {
